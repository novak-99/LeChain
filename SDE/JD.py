...
class JD:
    def __init__(self, S0, data, mu, sigma, dt):
        ...
        jump_mask = np.abs(data - self.mu) > 3 * self.sigma * np.sqrt(dt)

        num_jumps = int(jump_mask.sum())

        n = len(data)

        self.lam = num_jumps / (n * dt)  if num_jumps > 0 else 0

        if num_jumps > 1:
            Y = data[jump_mask] - self.drift * dt
            self.mu_j = Y.mean()
            self.sigma_j = Y.std()
        else:
            self.mu_j = 0
        ...
        if len(data_nojump > 10):
            self.sigma = data_nojump.std() / np.sqrt(dt)
    ...
    def sim(self, T, dt):
        ...
        for _ in range(T):
            Z = np.random.randn()
            N = np.random.poisson(self.lam * dt)
            ...
            S *= np.exp(self.drift * dt + self.sigma * np.sqrt(dt) * Z + total_jump)
            path.append(S)
